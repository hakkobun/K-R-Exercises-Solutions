/* modifying the example code for exercises 5-14 to 5-17 */
...
void kr_qsort(void *lineptr[], int left, int right, int (*comp)(void *, void *), int reverse);
...
int main(int argc, char *argv[])
{
    ...
    int fold = 0;           /* 1 if Case-folding (a == A) */
    ...
    if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
        ...
        kr_qsort((void **) lineptr, 0, nlines - 1, (int (*)(void *, void *))(numeric ? numcmp : (fold ? strcmpf : strcmp)), reverse);
...
/* compare two strings with case-folding */
int strcmpf(char *s1, char *s2)
{
    int i;
...
void kr_qsort(void *v[], int left, int right, int (*comp)(void *, void *), int reverse)
{
    int i, last;
    ...
    for (i = left + 1; i <= right; ++i)
        if (reverse * (comp)(v[i], v[left]) < 0)
            swap(v, ++last, i);
    swap(v, left, last);
    kr_qsort(v, left, last - 1, comp, reverse);
    kr_qsort(v, last + 1, right, comp, reverse);
}
